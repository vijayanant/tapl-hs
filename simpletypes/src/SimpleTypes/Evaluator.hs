...
deBruijn' ctx (Pair inf t1 t2)     = Pair inf (deBruijn' ctx t1) (deBruijn' ctx t2)
...
subst j s (Pair     inf t1  t2)      = Pair     inf (subst j s t1) (subst j s t2)
subst s j t                          = t
...
eval' (Pair inf t1 t2 ) = Right $ Pair inf (eval1 t1) (eval1 t2)
eval' t                     = Right t
